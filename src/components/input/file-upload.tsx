 ...
 (
  ...
  return (
   <div className="relative space-y-1 pb-3 pt-1">
    {label && (
     <label className="text-left text-sm font-normal capitalize" htmlFor={name}>
      {label}
     </label>
    )}
    <div
       ...
       <div className="relative h-[248px] w-full rounded-md transition-all duration-300">
        ...
        <Image
         src={
          ...
          value?.[0]?.url ? value?.[0]?.url : URL?.createObjectURL(value?.[0])
         }
         alt={`Preview`}
       ...
       >
        <div className="relative mx-auto my-auto flex h-28 w-80 flex-col items-center justify-center px-2 pb-6">
         ...
         <div className="pb-1 text-center">
          <Icon IconComp={Upload} boxSize={16} className="text-gray-700" />
         </div>
        </div>
       </button>
      )}
